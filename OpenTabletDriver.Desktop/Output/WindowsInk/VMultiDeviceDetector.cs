...
{
    // Ported from upstream PR #4866 (ooopus/avalonia-windows-ink-vmulti).
    // Detects the external VMulti VirtualHID kernel driver that Windows Ink
    // output writes to; we never bundle or install it ourselves.
    public static class VMultiDeviceDetector
    {
        ...
        public static VMultiDeviceStatus GetStatus()
        {
            try
            {
        ...
        public static bool TryOpenOutputDevice(out HidStream? stream, out VMultiDeviceStatus status)
        {
            stream = null;
        ...
        private static VMultiDeviceStatus GetStatusFromDevices(HidDevice[] devices, HidStream? openedOutputStream, bool requireOpenOutputStream)
        {
            if (devices.Length == 0)
            {
                return CreateStatus(
        ...
        private static VMultiDeviceStatus CreateStatus(
